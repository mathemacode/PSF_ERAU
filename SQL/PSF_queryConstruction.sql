...
SELECT IdentificationID, COUNT(DISTINCT(InternalCaseID)) AS NumCases
FROM PARTICIPANTS
...
-- Find number of cases particular IdentificationID appears in
-- With added info
SELECT IdentificationID, Gender, Ethnicity, COUNT(DISTINCT(InternalCaseID)) AS NumCases
FROM PARTICIPANTS
...
SELECT IdentificationID, COUNT(DISTINCT(InternalCaseID)) AS NumCases_Adult
FROM PARTICIPANTS
...
SELECT IdentificationID, COUNT(DISTINCT(InternalCaseID)) AS NumCases, RecordYear
FROM PARTICIPANTS
